...
class FiniteAET_filter(nn.Module):
    def __init__(self, config, filter_threshold, device="cuda"):
        ...
        # Satisficing lambda (signmoid를 통해 [0,1] 범위 강제)
        self.logit_lambda_sat = nn.Parameter(torch.tensor(-2.3).to(device)) # 초기값 lambda=0.5, nu=0.5
        ...
        self.ema_alpha = 0.99 # 묵직하게 추세를 따라가도록 설정
        ...
        self.dual_optim = optim.Adam([self.log_lambda_f_div, self.logit_lambda_sat], lr=config.nu_lr) #*0.1
    ...
    def update_nu_mu(self, states, next_states, timesteps, rewards, initial_states):
        ...
        # Loss 구성
        init_loss = init_nu.mean()
        loss_1 = (w * (e.reshape(-1)) - beta * f_vals).mean()
        loss_2 = (1-lambda_sat) * self.filter_threshold

        # Fair RL 관련 loss_3 (기존)
        scaled_mu = mu / (lambda_sat + 1e-8)
        if self.config.fair_alpha == 0.0:
            loss_3 = torch.tensor(0.0, device=states.device)
        elif self.config.fair_alpha == 1.0:
            loss_3 = piecewise_log_u_star_neg_mu(scaled_mu).sum()
        else:
            loss_3 = piecewise_frac_u_star_neg_mu(scaled_mu, self.config.fair_alpha).sum()

        # Total nu_loss
        nu_loss = init_loss + loss_1 + loss_2 + loss_3
        ...
        return k.detach(), f_vals.mean().detach(), w_notflat.detach(), {
            "nu_loss": nu_loss.item(),
            "pruning_rate": pruning_rate.item()
        }

    def update_meta_parameters(self, k, f_div):
        """
    ...
    def train_step(self, batch):
        ...
        k, f_vals, w_target, info_nu = self.update_nu_mu(states, next_states, timesteps, rewards, initial_states)
        # --- 2. Update λ (f-divergence constraint) ---
        info_meta = self.update_meta_parameters(k, f_vals)
        ...
        log_dict = {
            ...
            "train/lambda_sat": info_meta["lambda_sat"],
            "train/beta": info_meta["beta"],
            "train/ema_f_div": info_meta["ema_f_div"],
            "train/utility": info_meta["current_welfare"],
            "satisficing/gap": (info_meta["current_welfare"] - self.filter_threshold).item(),
        }
...
class FiniteAET_after(nn.Module):
    def __init__(self, config, filter_path = None, device="cuda"):
        ...
        self.nu_optim = optim.Adam(params_nu, lr=config.nu_lr)
        self.nu_scheduler = CosineAnnealingLR(self.nu_optim, T_max=config.num_steps, eta_min=1e-6)
        self.dual_optim = optim.Adam([self.log_lambda_f_div], lr=config.nu_lr) 
        self.dual_scheduler = CosineAnnealingLR(self.dual_optim, T_max=config.num_steps, eta_min=1e-6)
