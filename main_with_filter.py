...
from AET_filter import FiniteAET_filter, FiniteAET_after
...
def main():
    ...
    # ---- mode ----
    parser.add_argument("--mode", type=str, default="AET_filter", choices=["AET_filter"])
    ...
    if os.path.exists(filter_path) and not filter_test:
        ...
        if config.use_wandb:
            wandb.init(
                project=config.wandb_project + "_model",
                config=vars(config),
                name=model_name,
                entity = 'wsk208',
                tags = tag_name
        ...
        print("진입 확인!")
        agent = FiniteAET_after(config, filter_path, device=config.device)
        trainer(
            agent,
            buffer,
